Feature: Verify the buy amount with the conversion rate
  ...
  @Verify
  Scenario Outline: Verify the buy amount with the conversion rate from one currency to another.
    ...
    When User Creates a Conversion Quote to buy "<BuyCurrency>" and sell "<SellCurrency>" with "<FixedSide>" and "<Amount>" and "<agreement>"
